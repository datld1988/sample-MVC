...
import org.springframework.security.oauth2.common.VtcOAuth2Exception;
...
public abstract class AbstractOAuth2SecurityExceptionHandler {
	...
	protected final void doHandle(HttpServletRequest request, HttpServletResponse response, Exception authException)
		...
		try {
			...
			VtcOAuth2Exception exception = new VtcOAuth2Exception();
			exception.setError(String.valueOf(result.getStatusCode()));
			exception.setMessage(result.getBody().getMessage());
			
			HttpHeaders headers = new HttpHeaders();
			headers.set("Cache-Control", "no-store");
			headers.set("Pragma", "no-cache");
			headers.setContentType(MediaType.APPLICATION_JSON);
			ResponseEntity<VtcOAuth2Exception> resultVtc =  new ResponseEntity<VtcOAuth2Exception>(exception, headers, HttpStatus.OK);

			exceptionRenderer.handleHttpEntityResponse(resultVtc, new ServletWebRequest(request, response));
